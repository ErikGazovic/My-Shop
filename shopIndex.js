...
async function createTables(params) {
    ...
    await pool.query("CREATE TABLE IF NOT EXISTS cart (cart_id SERIAL PRIMARY KEY,product_id INT,user_id INT,size VARCHAR(2),CONSTRAINT fk_cart_product FOREIGN KEY (product_id) REFERENCES products(id),CONSTRAINT fk_cart_user FOREIGN KEY (user_id) REFERENCES users(id))");
    await pool.query("CREATE TABLE IF NOT EXISTS orders ( order_id SERIAL PRIMARY KEY, user_id INTEGER, order_price float, CONSTRAINT fk_users FOREIGN KEY (user_id) REFERENCES users(id)) )");
