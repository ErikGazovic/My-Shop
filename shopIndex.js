...
app.get("/home-page/:username", (req, res) => {
    ...
    res.render("shopIndex.ejs", {year: year, username: username});
});
...
app.get("/filter/targetGroup=:group&category=:category&type=:type", async (req, res) => {
    ...
    for (var i = 0; i < a.length; i++) {
        let newName = a[i];
        correspondingProductsNames = await db.query("SELECT * FROM products WHERE target_group = $1 AND category = $2 AND product_type = $3 AND product_name = $4", [group, category, type, newName]);
    ...
    let correspondingProducts = await db.query("SELECT * FROM products WHERE target_group = $1 AND category = $2 AND product_type = $3", [group, category, type]);
...
app.get("/:name/filter/targetGroup=:group&category=:category&type=:type/id=:id", async (req, res) => {
    ...
    for (var i = 0; i < a.length; i++) {
        let newName = a[i];
        correspondingProductsNames2 = await db.query("SELECT * FROM products WHERE target_group = $1 AND category = $2 AND product_type = $3 AND product_name = $4", [group, category, type, newName]);
    ...
    const correspondingProducts = await db.query("SELECT * FROM products WHERE target_group = $1 AND category = $2 AND product_type = $3", [group, category, type]);
    const clickedItem = await db.query(`SELECT * FROM products WHERE product_id = ${id}`);
...
app.post("/filter/targetGroup=:group&category=:category&type=:type&filtered-products", async (req, res) => {
    ...
    let correspondingProducts = await db.query("SELECT * FROM products JOIN product_storage ON products.product_id = product_storage.product_id WHERE target_group = $1 AND category = $2 AND product_type = $3", [group, category, type]);
    if (brand && color && price && size) {
        
        correspondingProducts = await db.query(`SELECT * FROM products JOIN product_storage ON products.product_id = product_storage.product_id WHERE target_group = $1 AND category = $2 AND product_type = $3 AND brand_name = $4 AND color = $5 AND price <= $6 AND ${size} > 0`, [group, category, type, brand, color, price]);
    } else if (brand && color && price && !size) {
        correspondingProducts = await db.query(`SELECT * FROM products JOIN product_storage ON products.product_id = product_storage.product_id WHERE target_group = $1 AND category = $2 AND product_type = $3 AND brand_name = $4 AND color = $5 AND price <= $6`, [group, category, type, brand, color, price]);
    } else if (brand && color && !price && size) {
        correspondingProducts = await db.query(`SELECT * FROM products JOIN product_storage ON products.product_id = product_storage.product_id WHERE target_group = $1 AND category = $2 AND product_type = $3 AND brand_name = $4 AND color = $5 AND ${size} > 0`, [group, category, type, brand, color]);
    } else if (brand && !color && price && size) {
        correspondingProducts = await db.query(`SELECT * FROM products JOIN product_storage ON products.product_id = product_storage.product_id WHERE target_group = $1 AND category = $2 AND product_type = $3 AND brand_name = $4 AND price <= $5 AND ${size} > 0`, [group, category, type, brand, price]);
    } else if (!brand && color && price && size) {
        correspondingProducts = await db.query(`SELECT * FROM products JOIN product_storage ON products.product_id = product_storage.product_id WHERE target_group = $1 AND category = $2 AND product_type = $3 AND color = $4 AND price <= $5 AND ${size} > 0`, [group, category, type, color, price]);
    } else if (brand && color && !price && !size) {
        correspondingProducts = await db.query(`SELECT * FROM products JOIN product_storage ON products.product_id = product_storage.product_id WHERE target_group = $1 AND category = $2 AND product_type = $3 AND brand_name = $4 AND color = $5`, [group, category, type, brand, color]);
    } else if (brand && !color && !price && size) {
        correspondingProducts = await db.query(`SELECT * FROM products JOIN product_storage ON products.product_id = product_storage.product_id WHERE target_group = $1 AND category = $2 AND product_type = $3 AND brand_name = $4 AND ${size} > 0`, [group, category, type, brand]);
    } else if (!brand && !color && price && size) {
        correspondingProducts = await db.query(`SELECT * FROM products JOIN product_storage ON products.product_id = product_storage.product_id WHERE target_group = $1 AND category = $2 AND product_type = $3 AND price <= $5 AND ${size} > 0`, [group, category, type, price]);
    } else if (brand && !color && price && !size) {
        correspondingProducts = await db.query(`SELECT * FROM products JOIN product_storage ON products.product_id = product_storage.product_id WHERE target_group = $1 AND category = $2 AND product_type = $3 AND brand_name = $4 AND price <= $5`, [group, category, type, brand, price]);
    } else if (!brand && color && !price && size) {
        correspondingProducts = await db.query(`SELECT * FROM products JOIN product_storage ON products.product_id = product_storage.product_id WHERE target_group = $1 AND category = $2 AND product_type = $3 AND ${size} > 0 AND color = $4`, [group, category, type, color]);
    } else if (!brand && color && price && !size) {
        correspondingProducts = await db.query(`SELECT * FROM products JOIN product_storage ON products.product_id = product_storage.product_id WHERE target_group = $1 AND category = $2 AND product_type = $3 AND price <= $4 AND color = $5`, [group, category, type, price, color]);
    } else if (!brand && !color && !price && size) {
        correspondingProducts = await db.query(`SELECT * FROM products JOIN product_storage ON products.product_id = product_storage.product_id WHERE target_group = $1 AND category = $2 AND product_type = $3 AND ${size} > 0`, [group, category, type]);
    } else if (brand && !color && !price && !size) {
        correspondingProducts = await db.query(`SELECT * FROM products JOIN product_storage ON products.product_id = product_storage.product_id WHERE target_group = $1 AND category = $2 AND product_type = $3 AND brand_name <= $4`, [group, category, type, brand]);
    } else if (!brand && color && !price && !size) {
        correspondingProducts = await db.query(`SELECT * FROM products JOIN product_storage ON products.product_id = product_storage.product_id WHERE target_group = $1 AND category = $2 AND product_type = $3 AND color = $4`, [group, category, type, color]);
    } else if (!brand && !color && price && !size) {
        correspondingProducts = await db.query(`SELECT * FROM products JOIN product_storage ON products.product_id = product_storage.product_id WHERE target_group = $1 AND category = $2 AND product_type = $3 AND price <= $4`, [group, category, type, price]);
    } else {
        correspondingProducts = await db.query("SELECT * FROM products JOIN product_storage ON products.product_id = product_storage.product_id WHERE target_group = $1 AND category = $2 AND product_type = $3", [group, category, type]);
    }
...
app.post("/:name/add-to-cart&id=:id", async (req, res) => {
    ...
    const userIDRaw = await db.query("SELECT user_id FROM users WHERE username = $1", [username]);
    const userID = userIDRaw.rows[0]["user_id"];
    await db.query("INSERT INTO cart (product_id, user_id, size) VALUES ($1, $2, $3)", [id, userID, size]);
...
app.get("/:name/my-cart", async (req, res) => {
    ...
    const userID = userIDRaw.rows[0]["user_id"];
    const sizesRaw = await db.query("SELECT size FROM cart WHERE user_id = $1", [userID]);
    const myItemsIdsRaw = await db.query("SELECT product_id FROM cart WHERE user_id = $1", [userID]);
    const myItemsIds = [];
    ...
    for (let itemId of myItemsIds) {
        allItemsInCart = await db.query("SELECT * FROM products WHERE product_id = $1", [itemId]);
...
app.post("/:name/purchase", async (req, res) => {
    ...
    await db.query("INSERT INTO orders (user_id, order_price) VALUES ($1, $2)", [userID, req.body["total-price"]]);
    await db.query("DELETE FROM cart WHERE user_id = $1", [userID]);


    const orderIDRaw = await db.query("SELECT order_id FROM orders WHERE user_id = $1", [userID]);
    const orderID = orderIDRaw.rows[0]["order_id"];


    const emailRaw = await db.query("SELECT email FROM users WHERE user_id = $1", [userID]);
...
app.post("/send-verification", async (req, res) => {
    ...
    const emailRaw = await db.query("SELECT * FROM users WHERE email = $1", [mailInput]);
...
app.post("/add-product", async (req, res) => {
    ...
    try {
        ...
        const resultNameColor = await db.query(`SELECT * FROM products WHERE product_name = '${product.productName}' AND color = '${product.productColor}'`);
        const resultProductImg = await db.query(`SELECT * FROM products WHERE image = '${product.productImg}'`);
        let failMsg;
        ...
        if (resultNameColor.rows.length > 0) {
            ...
            if (brandImageExists) {
                const brandExists = await db.query("SELECT * FROM brands WHERE brand_name = $1 OR brand_logo = $2", [product.productBrandName, product.productLogoImg]);
                let brandLogo;
                if (brandExists.rows.length < 1) {
                    await db.query("INSERT INTO brands (brand_name, brand_logo) VALUES ($1, $2)", [product.productBrandName, product.productLogoImg]);
                } 
                
                brandLogo = await db.query("SELECT * FROM brands WHERE brand_name = $1", [product.productBrandName]);
                const addProduct = await db.query("INSERT INTO products (image, product_name, target_group, category, product_type, brand_logo, brand_name, price, color) VALUES" +
                ...
                if (productSizes.length <= 8) {
                    await db.query("INSERT INTO product_storage (xxs, xs, s, m, l, xl, xxl, size27, size28, size29, size30, size31, size32, size33, size34, size35, size36, size37, size38, size39, size40, size41, size42, size43, size44, size45, size46, size47, product_id) VALUES" + 
                ...
                const logo = await db.query("SELECT * FROM brands WHERE brand_name = $1", [product.productBrandName]);
...
passport.use(new Strategy ({usernameField: "username", passwordField: "password"}, async function verify(username, password, cb) {
    try {
        ...
        const checkResultEmail = await db.query("SELECT * FROM users WHERE email = $1", [username]);
...
async function getFilterCriteria (group, category, type, criteria) {
    let listDemo = [];
    let cleanCriteria = [];
    let getCriteria = await db.query(`SELECT ${criteria} FROM products WHERE target_group = $1 AND category = $2 AND product_type = $3`, [group, category, type]);
