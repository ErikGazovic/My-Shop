...
let menuforWomen = [
  [
    "Clothing",
    ["Jackets", "Hoodies", "T-Shirts", "Jeans", "Pants", "Pyjamas", "Dresses"],
  ],
...
let menuforMen = [
  [
    "Clothing",
    ["Jackets", "Hoodies", "T-Shirts", "Jeans", "Pants", "Pyjamas", "Suits"],
  ],
...
let menuforBoys = [
  ["Clothing", ["Jackets", "Hoodies", "T-Shirts", "Jeans", "Pants", "Pyjamas"]],
  ["Shoes", ["Boots", "Sneakers", "Trainers", "Slippers", "Elegant Shoes"]],
  ["Accessories", ["Hats", "Glasses", "Underwear", "Jewelry"]],
];

let menuforUnisex = [
  ["Clothing", ["Jackets", "Hoodies", "T-Shirts", "Pants", "Pyjamas"]],
  ["Shoes", ["Boots", "Sneakers", "Trainers", "Slippers"]],
  ["Accessories", ["Hats", "Glasses", "Jewelry"]],
];

app.get(
  "/filter/targetGroup=:group&category=:category&type=:type",
  async (req, res) => {
    ...
    switch (group) {
      case "Women":
        listToSelect = menuforWomen;
        break;
      case "Men":
        listToSelect = menuforMen;
        break;
      case "Girls":
        listToSelect = menuforGirls;
        break;
      case "Boys":
        listToSelect = menuforBoys;
        break;
      case "Unisex":
        listToSelect = menuforUnisex;
        break;
      default:
        break;
    }
    ...
    let allBrands = await getFilterCriteria(
      group,
      category,
      type,
      "brand_name"
...
app.get(
  "/:name/filter/targetGroup=:group&category=:category&type=:type",
  async (req, res) => {
    const username = req.params.name;
    const group = req.params.group;
...
app.get(
  "/:name/filter/targetGroup=:group&category=:category&type=:type/id=:id",
  async (req, res) => {
    const username = req.params.name;
    ...
    for (var i = 0; i < a.length; i++) {
      ...
      if (correspondingProductsNames.rowCount > 0) {
        for (let correspondingProduct of correspondingProductsNames.rows) {
          imagesAndIDsAndNames.push([
            newName,
            [correspondingProduct.image, correspondingProduct["product_id"]],
          ]);
        }
      }
    }
    ...
    res.render("products.ejs", {
      ...
      type: type,
      id: id,
      clickedItem: clickedItem.rows[0],
      correspondingProducts: correspondingProducts.rows,
...
app.get(
  "/filter/targetGroup=:group&category=:category&type=:type/id=:id",
  async (req, res) => {
    const group = req.params.group;
    const id = req.params.id;
    const category = req.params.category;
    ...
    for (var i = 0; i < a.length; i++) {
      ...
      if (correspondingProductsNames.rowCount > 0) {
        for (let correspondingProduct of correspondingProductsNames.rows) {
          imagesAndIDsAndNames.push([
            newName,
            [correspondingProduct.image, correspondingProduct["id"]],
          ]);
        }
      }
    }
...
app.post(
  "/filter/targetGroup=:group&category=:category&type=:type&filtered-products",
  async (req, res) => {
    const brand = req.body["filter-brand"];
    const colorLowerCase = req.body["filter-color"];
    const color =
      String(colorLowerCase).charAt(0).toUpperCase() +
      String(colorLowerCase).slice(1);
...
app.post(
  "/:name/filter/targetGroup=:group&category=:category&type=:type&filtered-products",
  async (req, res) => {
    const brand = req.body["filter-brand"];
    const colorLowerCase = req.body["filter-color"];
    const color =
...
app.post("/:name/add-to-cart&id=:id", async (req, res) => {
  ...
  res.redirect(
    url.format({
      pathname: `/${username}/filter/targetGroup=${group}&category=${category}&type=${type}/id=${id}`,
      query: {
        success: successMsg,
      },
    })
...
app.get("/:name/my-cart", async (req, res) => {
  ...
  for (let itemId of myItemsIds) {
    allItemsInCart = await pool.query("SELECT * FROM products WHERE id = $1", [
      itemId,
    ]);
    ...
    if (user) {
      ...
      if (options.message === "Wrong password") {
        res.render("login.ejs", {
          passwordMessage: options.message,
          year: year,
        });
      } else {
  ...
  new Strategy(
    ...
    async function verify(username, password, cb) {
      try {
        ...
        if (
          checkResultUsername.rows.length > 0 ||
          checkResultEmail.rows.length > 0
        ) {
...
function checkRegisterPassword(password, reppeatedPassword) {
  ...
  if (password != reppeatedPassword) {
    ...
    let passwordMessage =
      "Password must have at least 1 of these symbols: +, -, (, ), &, #, %, $";
    return [false, passwordMessage];
  } else if (lowerCaseCount < 1) {
    let passwordMessage =
      "Password must contains at least 1 lower cased character";
    return [false, passwordMessage];
  } else if (upperCaseCount < 1) {
    let passwordMessage =
      "Password must contains at least 1 upper cased character";
    return [false, passwordMessage];
